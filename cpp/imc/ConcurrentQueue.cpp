...
int test()
{
    ...
    ConcurrentQueue<Element*,queueSize> queue;

    for (int i = 0; i < queueSize; ++i)
    {
        queue.push(new Element(i));
    }

    int value = 10;
    while (true)
    {
        Element * pushed = new Element(value++);
        Element * popped;

        std::thread consumer([ & ] {
                    popped = queue.pop();
                    std::cout << "popped " << popped->mValue << std::endl;
                });

        std::thread producer([ & ] {
                    queue.push(pushed);
                    std::cout << "pushed " << pushed->mValue << std::endl;
                });

        consumer.join();
        producer.join();

        assert(pushed->mValue != popped->mValue);
        delete popped;
    }
    return 0;
}
